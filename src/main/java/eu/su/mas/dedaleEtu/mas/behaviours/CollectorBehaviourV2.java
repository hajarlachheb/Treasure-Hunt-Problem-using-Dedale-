...
public class CollectorBehaviourV2 extends TickerBehaviour {
    ...
    public int StuckIterThr2 = 5;
    public int StuckIterRandomLimit = 2;
    ...
    public CollectorBehaviourV2 (final AbstractDedaleAgent myagent) {
    ...
    public void onTick() {
        ...
        if (myPosition != "") {
                ...
                if (lockStatus == 1) {
                    ...
                    if (treasureGrabbed > 0)
                        System.out.println(this.myAgent.getLocalName() + " - Grabbed " + treasureGrabbed +
                                ...
                                spaceAfter + ") | Observations: " +((AbstractDedaleAgent) this.myAgent).observe());
                    this.amountCollected += treasureGrabbed;

                    /*********************************
                     * UPDATE COLLECTING LIST
                     ********************************/

                    //Observe the position again
                    List<Couple<Observation,Integer>> lObsAfterGrab= ((AbstractDedaleAgent) this.myAgent).observe().get(0).getRight();

                    //Get the new parameters
                    Observation treasureTypeAfterGrab = Observation.NO_TREASURE;
                    int amountAfterGrab = -1;

                    for(Couple<Observation,Integer> o:lObsAfterGrab){
                        switch (o.getLeft()) {
                            case DIAMOND:case GOLD:
                                treasureTypeAfterGrab = o.getLeft();
                                amountAfterGrab = o.getRight();
                                break;
                            default: break;
                        }
                    }

                    // When the treasure is completely collected, we only get the lockpicking, strength and lockstatus parameters
                    // Check if the treasure type observation was removed to verify that it was collected
                    if (treasureTypeAfterGrab.equals(Observation.NO_TREASURE)) {
                        updateCollectedList(lobs, myPosition);
                    }
                    else if (treasureTypeAfterGrab.equals(myStatus.getTreasureType())) {
                        System.out.println(this.myAgent.getLocalName() + " - My backpack is full. There are " +
                                amountAfterGrab + " of " + treasureTypeAfterGrab + " here");
                        ...
                        System.out.println(this.myAgent.getLocalName() + " - I cannot collect this treasure. There are " +
                                amountAfterGrab + " of " + treasureTypeAfterGrab + " here");
            ...
            // Update my agent status
            if (myStatus != null) {
                //System.out.print(this.myAgent.getLocalName() + " - Previous status: " + myStatus.SerializeContent());
                for(Couple<Observation,Integer> o:((AbstractDedaleAgent) this.myAgent).getBackPackFreeSpace()){
                    if (myStatus.treasureType == o.getLeft())
                        myStatus.setBackpackCapacity(o.getRight());
                }
                //System.out.println(" | New status: " + myStatus.SerializeContent());
            }
            ...
            if ((waitingCounter > 0)&&(!performTask)){
                ...
                //System.out.println(this.myAgent.getLocalName() + " - Waiting to perform a task (" + waitingCounter + ")");
                waitingCounter--;
        ...
        if(path2treasure.size() > 0) {
            ...
            if(path2treasure.size() > 0) {
                ...
                if (validPath) {
                    ...
                    Random r = new Random();
                    int StuckIterRand = r.nextInt(0, this.StuckIterRandomLimit);
                    if (this.StuckIter > this.StuckIterThr2 + StuckIterRand)
                        this.StuckRecover = this.RecoverIterThr;

                    if ((this.StuckIter > this.StuckIterThr2 + StuckIterRand) || (this.StuckRecover > 0)) {
                        // Random movement
                        nextNode = randomMovement(lobs);
                        path2treasure.add(0, myPosition);
                    ...
                    System.out.println(this.myAgent.getLocalName() + " - found a not valid path | lobs: " + lobs + " | path: " + path2treasure);
                    performTask = false;
                }
            }
        }
